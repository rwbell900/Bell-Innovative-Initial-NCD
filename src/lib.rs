...
//      manufacturers website.  The contract is deployed with the wholesale destributor contract is in applied status.
...
//  3.  The wholesale destributor then purchases inventory of bags and cases of product that the manufacturer can 
//      hold and drop-ship as orders are made from the wholesale destributors website.
//  4.  Orders are then placed on the wholesale destributors website and " and paid for either in NEAR or by credit card" payment
//      will be implemented in a later version of this contract.
//  5.  The order is added to the list of orders for the wholesale destributor.  The status of the order at that time will
//      be "ordered".
...
//  9.  In future versions of this contract I invision adding a "Factory" contract and adding a collection of wholesale destributors to this 
//      contract, or creating multiple instances of this contract that exist only as long as the wholesale customer/distributor is active.
//  10. The unit test for this contract creates the contract puts the wholesale destributor in "Apply" status, updates them to "Approved" status
//      then adds an order to the order list.  Also all get and set methods are tested in addition to an encription method for storing
//      account information encripted.  Then verifying that the account Id matches the encripted value stored.
...
// A HashMap of Orders using this structure is in the Contract for the Wholesale destributor That is unique to each contract instance.
    #[near_bindgen]
...
//  One contract will exist for each Wholesale Distributor
#[near_bindgen]
#[derive(BorshDeserialize, BorshSerialize, PanicOnDefault)]
pub struct Contract { //main contract
    // SETUP CONTRACT STATE
    ws_entity_account: String, // wholesale distributor account
    ws_entity_main_account: AccountId, // AccountId
    ws_entity_name: String, //wholesale distributor entity name
    ws_entity_status: String, //wholesale distributor status: Currently either "Applicant" or "Approved"
    ws_entity_description: String, //wholesale distributor entity description
    ws_entity_allowed_inventory_bags: u8,//The number of bags of inventory wholesale distributor is allowed to have in inventory for dropship
    ws_entity_allowed_inventory_cases: u8, //the number of cases of product that the wholesale distributor is allowed to inventory for dropship
    ws_bag_inventory: u8, // the number of bags currently in inventory for this wholesale distributor
    ws_case_inventory: u8, // the number of cases currently in inventory for this wholesale distributor
    ws_entity_owner: std::string::String, // the wholesale distributor owner name
    ws_customer_orders : HashMap<u64, EndCustomerOrder>,// HashMap list of orders placed for dropshipment by customers of the wholesale distributor
}
...
impl Contract{
    ...
    pub fn new() -> Self
        ...
        {
            // if the Wholesale destributors status is "Approved" the destributor is allowed to receive new orders
    ...
    // set function for the Wholesale destributor entity name
    pub fn set_entity_name(&mut self, name: String) {
        self.ws_entity_name = name;
    }
    
    // get function for the Wholesale destributor entity name
    ...
    pub fn set_entity_status(&mut self, status: String) {
        // If the wholesale destributor is currently and "Applicant" and a request is being made 
        // Change the destributors status to Approved then allow the status change.
        if self.ws_entity_status == "Applicant"{
            ...
            // if the Wholesale destributor is set to status "nostat"
    ...
    // set function for current bag inventory for this wholesale destributor
...
mod tests {
    ...
    // Wholesale destributor.  Then to create a new order for the Wholesale destributor and ad it to the 
    // Wholesale destributor Contract
    fn debug_check_contract() {
        ...
        let entity_name = "Alices Wholesale Garden Products";
        let entity_status = "Applicant";
        let entity_description = "We supply the best all natural garden products";
        let entity_allowed_inventory_bags = 40 as u8;
        let entity_allowed_inventory_cases = 20 as u8;
